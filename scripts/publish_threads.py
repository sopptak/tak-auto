...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.auto:
        try:
            selection = select_unpublished_threads_item(valid_items, history)
        except PublishHistoryError as err:
            print(f"오류: {err}", file=sys.stderr)
            return 1

        if selection is None:
            print("게시할 콘텐츠 없음: 검증 통과했지만 아직 게시하지 않은 Threads 콘텐츠가 없습니다.")
            return 0

        target_item, content_id = selection
        selection_label = f"자동 선택 (게시 이력에 없는 첫 valid 항목, content_id={content_id})"
    else:
        ...
        try:
            content_id = compute_content_id(target_item)
        except PublishHistoryError as err:
            print(f"오류: {err}", file=sys.stderr)
            return 1
