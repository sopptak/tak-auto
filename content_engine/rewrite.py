...
_FACT_RISK_TERMS = frozenset(
    {
        # "경험"은 6-32에서 제외했다 - 다른 항목(성과/수익/매출/계약/투자 등)과
        # 달리 그 자체로 구체적인 새 사실·주장을 나타내지 않는 일반 서술 단어라서,
        # source_text에 "경험"이라는 글자가 우연히 없으면(예: knowledge_type이
        # "의견"인 SCOUT 경로) 사실 왜곡이 전혀 없는 재작성도 false positive로
        # 차단됐다(재현: 원문/재작성 모두 같은 사실만 담고 있어도 "제가 이 도구를
        # 써 본 경험을 나눕니다"라는 표현만으로 거부됨). 나머지 항목은 전부
        # 구체적인 성과/금액/계약/기관 등 실제 새 사실 주장에 해당하므로 그대로
        # 유지한다 - validator를 느슨하게 만들어 새로운 사실을 통과시키지 않는다.
        "성과", "수익", "매출", "고객", "출시", "수상", "계약", "투자",
